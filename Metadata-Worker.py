...
def compare_metadata_files(file1: str, file2: str, bytes_count: int = 10) -> bool:
    if not os.path.isfile(file1):
        print(f"{Fore.RED}Error: File {file1} does not exist.{Style.RESET_ALL}")
        return False
    if not os.path.isfile(file2):
        print(f"{Fore.RED}Error: File {file2} does not exist.{Style.RESET_ALL}")
        return False
    ...
    print(f"\n{Fore.CYAN}Bytes from {file1}:{Style.RESET_ALL} " + " ".join(f"{b:02x}" for b in data1))
    print(f"{Fore.CYAN}Bytes from {file2}:{Style.RESET_ALL} " + " ".join(f"{b:02x}" for b in data2))
    ...
    if data1 == data2:
        print(f"{Fore.GREEN}The first {bytes_count} bytes are identical.{Style.RESET_ALL}")
        return True
    else:
        different = [i + 1 for i in range(bytes_count) if data1[i] != data2[i]]
        print(f"{Fore.YELLOW}Bytes {different} are different - possible protection at offset(s) {different}{Style.RESET_ALL}")
        return False
